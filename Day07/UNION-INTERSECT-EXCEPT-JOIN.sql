...
select state,MAX(salary) AS total_salary
from workers
GROUP BY state
...
FROM workers
ORDER BY length(CONCAT(name,state))  --> ORDER BY length(name) + length(state);

--2.yol:
select name ||' '||state||' '||length(name) + length(name) AS name_and_state
from workers 
...
select mc.company_id,o.order_id,o.order_date
